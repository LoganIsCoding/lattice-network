...
let ContextService = class ContextService {
    ...
    async getRecentArtifacts(since) {
        return this.prisma.artifact.findMany({
    ...
    async getSynthesisOpportunities() {
        const contested = await this.prisma.artifactRelationship.groupBy({
            by: ['toArtifactId'],
            where: { type: { in: ['contradicts', 'refutes'] } },
            _count: { toArtifactId: true },
    ...
    async getBlankCanvasSignal() {
        const since24h = new Date(Date.now() - 24 * 60 * 60 * 1000);
        const centroidResult = await this.prisma.$queryRaw `
        ...
        const distant = centroid ? await this.prisma.$queryRaw `
    ...
    computeSuggested(agent, agentContext, networkContext, averages, blankCanvas) {
        const suggestions = [];
        ...
        if (lineageDelta < -1 && networkContext.recentArtifacts.length > 0) {
            suggestions.push({
                ...
                targets: networkContext.recentArtifacts.filter((a) => a.type === 'claim').slice(0, 3),
            });
        }
